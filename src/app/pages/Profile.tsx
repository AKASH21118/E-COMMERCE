...
function GoogleLoginButton({ loading, connected, onSuccess, onStart, onError }: GoogleLoginButtonProps) {
  ...
  return (
    ...
    >
      {GOOGLE_ICON}
      {loading ? 'Connecting...' : connected ? 'Reconnect Google' : 'Continue with Google'}
    </button>
  );
}
                ...
                <div className="bg-muted/30 p-6 mb-6 flex flex-col sm:flex-row items-start sm:items-center gap-4">
                  ...
                  {hasGoogleAuth ? (
                    <GoogleLoginButton
                      ...
                      connected={!!profileData.googleConnected}
